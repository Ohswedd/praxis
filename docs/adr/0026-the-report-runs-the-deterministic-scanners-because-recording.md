...
## Consequences
Recording a report is now slower, by the cost of three scans over the change. A report written by 3.1 carries no scan evidence and is rejected rather than grandfathered, on the same reasoning as ADR-0010: an unverifiable claim is not evidence, and re-recording is one command. The gate duplicates the verdict logic that report.py already applies, deliberately: the report is JSON in a directory anyone can write, so a gate that trusted its status field would have its whole guarantee one hand-edited line away.
