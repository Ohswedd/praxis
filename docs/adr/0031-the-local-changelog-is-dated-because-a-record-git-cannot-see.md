...
## Consequences
A new state file, listed in docs/STABILITY.md. A contributor whose local record predates this branch is asked to write the entry again, which is correct rather than merely stricter. Two bounded residues, both documented: a commit date has one-second resolution while a write is stamped to the microsecond, so an entry written moments before its commit counts as part of it, which is the right direction to be wrong in; and a rebase moves the base forward and can age out an earlier entry, where writing it again is what the branch's history now says happened. Owner mode and contributor repos that have their own changelog are untouched, since git already answers those exactly.
