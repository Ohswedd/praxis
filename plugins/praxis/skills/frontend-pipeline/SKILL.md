...
- markup and templates (`.html`, `.jsx`, `.tsx`, `.vue`, `.svelte`, `.astro`,
  `.erb`, `.twig`, and the rest);
...
A `patch` is still designed work: it inherits the existing system rather than
skipping design. If the repo has no `docs/design/` at all, a "small" change has
nothing to be consistent with, so establish the minimal brief and token set first
(treat those pieces as `full`) instead of inventing one-off values that the next
change will have to live with.
