...
- **No AI/tool attribution.** Do not add a `Co-Authored-By` trailer for Claude/an
  AI, or a "generated by" credit, to the commit message. The history is the
  project's own. (Describing the platform — "praxis is a Claude Code plugin" — is
  fine; crediting authorship is not.)
...
the release automation reads it. Do not add an AI "generated by" footer to the
body (same rule as commits).
