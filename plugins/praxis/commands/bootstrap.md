...
skill: map the codebase read-only, generate or reconcile the CLAUDE.md hierarchy
(route any legacy file through `@praxis:claudemd-verifier`), propose
`.claude/settings.json` guardrails, and propose LSP/MCP wiring via
`capability-discovery`. Show every change as a diff and confirm before writing.
